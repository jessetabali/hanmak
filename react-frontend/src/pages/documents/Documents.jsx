...
const STATUS_COLORS = { ready: 'success', processing: 'warning', uploaded: 'secondary', failed: 'danger' };
...
export default function Documents() {
  ...
  const { data: summaryData } = useApiQuery(['documents-summary'], EP.DOCUMENTS + 'summary/');
  ...
  const uploadMutation = useApiMutation(
    (formData) => apiClient.post(EP.DOCUMENTS, formData, { headers: { 'Content-Type': 'multipart/form-data' } }),
    {
      invalidateKeys: ['documents', 'documents-summary'],
      onSuccess: async (res) => {
        const doc = res.data;
  ...
  const scanMutation = useApiMutation(
    (id) => apiClient.post(EP.DOCUMENT_SCAN(id), {}),
    {
      invalidateKeys: ['documents'],
      onSuccess: () => toast.success('Document scanned'),
      onError: (e) => toast.error(e.response?.data?.detail || e.message),
    }
  );
  ...
  const deleteMutation = useApiMutation(
    ...
    {
      invalidateKeys: ['documents', 'documents-summary'],
      onSuccess: () => { toast.success('Document deleted'); setConfirmDelete(null); setSelectedDoc(null); },
      onError: (e) => toast.error(e.response?.data?.detail || e.message),
    }
  );
  ...
  const openUploadModal = () => { setUploadModal(true); setUploadFile(null); setUploadTitle(''); };

  const getExt = (doc) => {
    if ((doc.mime_type || '').includes('pdf')) return 'PDF';
    return (doc.title || doc.original_filename || '').split('.').pop().toUpperCase() || 'FILE';
  };
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Uploaded documents and stored files</p>
        </div>
        <div className="flex gap-2">
          <button className="btn btn-ghost" onClick={() => refetch()}>Refresh</button>
          <button className="btn btn-primary" onClick={openUploadModal}>Upload</button>
        </div>
      </div>
      ...
      <div className="stats-grid" style={{ '--cols': 4, marginBottom: '1.5rem' }}>
        {[
          ...
          ['Storage Used', formatBytes(summary.file_size ?? 0)],
        ].map(([label, value]) => (
      ...
      <div className="card" style={{ padding: '0.75rem 1rem', marginBottom: '1rem' }}>
        <div className="table-toolbar" style={{ gap: '0.5rem' }}>
          ...
          />
          <select className="form-input" style={{ width: 160 }} value={sort} onChange={(e) => { setSort(e.target.value); setPage(1); }}>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          action={<button className="btn btn-primary" onClick={openUploadModal}>Upload Document</button>}
        />
      ) : (
            ...
            <tbody>
              {docs.map((doc) => (
                <tr key={doc.id} style={{ cursor: 'pointer' }} onClick={() => setSelectedDoc(doc)}>
                  <td>
                    <div style={{ fontWeight: 600 }}>{doc.title || doc.original_filename || '—'}</div>
                  </td>
                  <td>
                    <span style={{ fontSize: '0.75rem', padding: '0.15rem 0.4rem', background: 'var(--bg-secondary)', borderRadius: 4, fontWeight: 600 }}>
                      {getExt(doc)}
                    </span>
                  </td>
                  ...
                  <td>
                    <Badge color={STATUS_COLORS[doc.status] || 'secondary'}>{doc.status || '—'}</Badge>
                  </td>
                  <td style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>{formatDate(doc.created_at)}</td>
                  <td onClick={(e) => e.stopPropagation()}>
                    <div className="flex gap-1">
                      <button
                        className="btn btn-ghost btn-sm"
                        title="Prepare for Builder"
                      ...
                      <button
                        ...
                        onClick={() => duplicateMutation.mutate({ id: doc.id, title: `${doc.title || 'Document'} Copy` })}
                      >
                        Copy
                      </button>
                      <button
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button className="btn btn-primary" disabled={uploadMutation.isPending || !uploadFile} onClick={handleUpload}>
      ...
      >
        <div className="form-group">
          ...
          <input className="form-input" placeholder="Document title" value={uploadTitle} onChange={(e) => setUploadTitle(e.target.value)} />
        </div>
        <div className="form-group">
          <div
          ...
          >
            ...
            {uploadFile ? (
              <div>
                ...
                <div style={{ fontSize: '0.8125rem', color: 'var(--text-muted)' }}>{formatBytes(uploadFile.size)}</div>
              </div>
            ) : (
              <div>
                <div style={{ fontWeight: 500, marginBottom: '0.25rem' }}>Drop file here or click to browse</div>
                <div style={{ fontSize: '0.8125rem', color: 'var(--text-muted)' }}>PDF, DOCX, or images</div>
              </div>
            )}
            <input
              ref={fileInputRef}
      ...
      <Drawer open={!!selectedDoc} onClose={() => setSelectedDoc(null)} title={selectedDoc?.title || 'Document'} width={480}>
        {selectedDoc && (
          <div className="flex flex-col gap-3">
            <Badge color={STATUS_COLORS[selectedDoc.status] || 'secondary'}>{selectedDoc.status || '—'}</Badge>

            {[
              ['File Type', getExt(selectedDoc)],
            ...
            {selectedDoc.file_url && (
              <a className="btn btn-ghost btn-sm" href={selectedDoc.file_url} target="_blank" rel="noreferrer" style={{ alignSelf: 'flex-start' }}>
                Download File
              </a>
            )}
            ...
            <div className="flex flex-col gap-2" style={{ marginTop: '0.5rem' }}>
              ...
              <button
                ...
                onClick={() => scanMutation.mutate(selectedDoc.id)}
              >
                Scan Document
              </button>
              <button
                className="btn btn-ghost btn-sm"
                disabled={duplicateMutation.isPending}
                onClick={() => duplicateMutation.mutate({ id: selectedDoc.id, title: `${selectedDoc.title || 'Document'} Copy` })}
              >
                Duplicate
              </button>
              <button
